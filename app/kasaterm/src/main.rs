...
impl ViewerLaunch {
    fn detect(executable: Option<&std::ffi::OsStr>, mut args: Vec<std::ffi::OsString>) -> Self {
        ...
        // KasaLite 는 라이트 모드 하나뿐이다 — 실행 파일 이름·인자와 무관하게 늘 lite.
        let _ = (named_viewer, named_lite, flagged, flagged_lite);
        Self {
            viewer_only: false,
            lite: true,
...
mod viewer_launch_tests {
    ...
    #[test]
    fn every_launch_is_lite_and_never_viewer() {
        for name in ["kasaterm", "kasaterm-lite", "kasaterm-viewer"] {
            let launch = ViewerLaunch::detect(Some(OsStr::new(name)), vec!["--viewer".into(), "note.md".into()]);
            assert!(launch.lite && !launch.viewer_only);
            assert_eq!(launch.paths, vec![std::path::PathBuf::from("note.md")]);
        }
    }
}
